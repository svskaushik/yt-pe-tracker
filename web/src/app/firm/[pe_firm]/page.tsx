...
export default async function FirmPage({ params }: FirmPageProps) {
  ...
  const totalSubscribers = firmChannels.reduce((sum, channel) => 
    sum + (channel.subscriber_count || 0), 0
  );
  const totalDeals = firmChannels.filter(channel => 
    channel.deal_value && channel.deal_value > 0
  ).length;
  const totalValue = firmChannels.reduce((sum, channel) => 
    sum + (channel.deal_value || 0), 0
  );
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <div className="mb-8">
            ...
            <p className="text-muted-foreground">
              No channels found for this PE firm.
            </p>
          </div>
    ...
    <div className="min-h-screen bg-background">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="mb-8">
          ...
          <p className="text-muted-foreground">
            YouTube channel acquisitions and investments
          </p>
        </div>
        ...
        <div className="bg-card rounded-lg border border-border p-8 text-center">
          ...
          <p className="text-muted-foreground mb-6 max-w-2xl mx-auto">
            Help us keep this database accurate and up-to-date by submitting additional acquisitions or corrections.
          </p>
          <Link 
            href="/submit"
            className="inline-flex items-center gap-2 bg-primary text-primary-foreground px-6 py-3 rounded-lg font-semibold hover:bg-primary/90 transition-colors"
